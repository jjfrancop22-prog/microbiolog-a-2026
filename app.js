...
async function migrate(){const all=await idbAll('records');let seq=1;for(const row of all.filter(x=>x.domain==='mediaPrep'&&!x.deleted).sort((a,b)=>(a.data?.date||'').localeCompare(b.data?.date||''))){const p={...row.data};let changed=false;if(!p.internalCode){p.internalCode=`MED-${String(p.date||today()).slice(0,4)}-${String(seq).padStart(6,'0')}`;changed=true}seq++;if(p.sterilityDueDate!==p.date){p.sterilityDueDate=p.date;changed=true}if(!p.macroscopicDueDate){p.macroscopicDueDate=p.date;changed=true}if(changed)await idbPut('records',{...row,data:p})}for(const row of all.filter(x=>x.domain==='catalogBottles'&&!x.deleted)){const b={...row.data};if(!b.operationalStatus){b.operationalStatus='ACTIVO';await idbPut('records',{...row,data:b})}}const cfg=all.find(x=>x.key==='systemConfig:media-control')?.data;if(cfg&&cfg.alertDays===undefined)await idbPut('records',{key:'systemConfig:media-control',domain:'systemConfig',data:{...cfg,alertDays:2},deleted:false});for(const row of all.filter(x=>x.domain==='catalogMedia'&&!x.deleted)){const m={...row.data};let changed=false;if(!m.technicalClass){m.technicalClass=defaultTechnicalClass(m.name);changed=true}const recommendedLife={'AS1':7,'AC1':7,'LMX':7,'PDA':15,'PCA':15,'EMB':15,'AN':15,'BHI':15}[m.prefix];const legacyLife={'PDA':14,'PCA':14,'EMB':14,'AN':14,'BHI':7}[m.prefix];if(recommendedLife!==undefined&&(m.shelfLifeDays===undefined||m.shelfLifeDays===null||Number(m.shelfLifeDays)===Number(legacyLife))){m.shelfLifeDays=recommendedLife;changed=true}if(!m.performanceProfile){const d=DEFAULT_MEDIA.find(x=>x.name===m.name);if(d?.performanceProfile){m.performanceProfile=structuredClone(d.performanceProfile);changed=true}}if((m.id==='medium-lmx'||m.prefix==='LMX'||m.name==='LMX Fluorocult')&&m.performanceProfile?.selectivity?.strainId!=='strain-sal-14028'){m.performanceProfile={...m.performanceProfile,selectivity:{...(m.performanceProfile?.selectivity||{}),strainId:'strain-sal-14028'}};changed=true}if(changed)await idbPut('records',{...row,data:m})}for(const row of all.filter(x=>x.domain==='performanceTasks'&&!x.deleted)){const t={...row.data};if(t.medium==='LMX Fluorocult'&&t.status==='PENDIENTE'&&t.profileSnapshot?.selectivity?.strainId!=='strain-sal-14028'){t.profileSnapshot={...t.profileSnapshot,selectivity:{...(t.profileSnapshot?.selectivity||{}),strainId:'strain-sal-14028'}};await idbPut('records',{...row,data:t})}}for(const row of all.filter(x=>x.domain==='performanceTests'&&!x.deleted)){const test={...row.data};if(test.testType!=='selectivity'||test.result!=='PENDIENTE')continue;const taskRow=all.find(x=>x.domain==='performanceTasks'&&!x.deleted&&x.data?.id===test.taskId);if(taskRow?.data?.medium==='LMX Fluorocult'&&test.strainId!=='strain-sal-14028'){test.strainId='strain-sal-14028';await idbPut('records',{...row,data:test})}}for(const row of all.filter(x=>x.domain==='mediaPrep'&&!x.deleted)){const p={...row.data};if(p.criteriaSnapshot&&!p.criteriaSnapshot.technicalClass){p.criteriaSnapshot={...p.criteriaSnapshot,technicalClass:defaultTechnicalClass(p.medium)};await idbPut('records',{...row,data:p})}}
  // A3.7: una sola cepa maestra por referencia ATCC. Los lotes comerciales viven en Trazabilidad.
  const strainRows=all.filter(x=>x.domain==='catalogStrains'&&!x.deleted);
  const byRef=new Map();
  for(const row of strainRows){const ref=String(row.data?.referenceCode||'').trim().toUpperCase();if(!ref)continue;(byRef.get(ref)||byRef.set(ref,[]).get(ref)).push(row)}
  for(const [ref,rows] of byRef){if(rows.length<2)continue;const canonical=rows.find(r=>r.data?.id==='strain-ec-25922')||rows.find(r=>!/-\d+$/.test(String(r.data?.name||'')))||rows[0];for(const dup of rows.filter(r=>r!==canonical)){for(const pr of all.filter(x=>x.domain==='productCatalog'&&!x.deleted&&x.data?.erpStrainId===dup.data.id)){await idbPut('records',{...pr,data:{...pr.data,erpStrainId:canonical.data.id}})}await idbPut('records',{...dup,deleted:true,data:{...dup.data,deleted:true,mergedIntoStrainId:canonical.data.id,deletedReason:'DUPLICADO DE CEPA MAESTRA POR REFERENCIA ATCC'}})}}
}
...
function qcNextDueFor(type,medium){const last=qcControlsOf(type,medium)[0];return last?qcNextFromActual(type,last.actualDate):qcGenerateNextFromAnchor(type,today())}
...
function qcDependency(type,date){const phase=qcPhase(type,date),a1=qcA1Lots(date),lmx=qcLMXLots(date),reacts=qcEligibleReactivations(type,phase,date),reactsTotal=qcEligibleTotalReactivations(type,phase,date),strain=qcStrainFor(type,phase),strainTotal=qcTotalStrainFor(type,phase);const samePositive=phase==='POSITIVE'&&strain?.id===strainTotal?.id;const a1Ready=a1.length>0&&(type==='Q3'||reacts.length>0);const lmxReady=lmx.length>0&&(type==='Q3'||(samePositive?reacts.length>0:reactsTotal.length>0));return {phase,a1,lmx,reacts,reactsTotal,strain,strainTotal,samePositive,a1Ready,lmxReady,ready:a1Ready||lmxReady} }
function qcPlanItem(type){const dueA1=qcNextDueFor(type,'A1'),dueLMX=qcNextDueFor(type,'LMX'),due=[dueA1,dueLMX].sort()[0],dep=qcDependency(type,due),days=qcDayDiff(today(),due);let status=days<0?'VENCIDO':days===0?'HOY':days<=3?'PRÓXIMO':'PROGRAMADO';const ready=(dueA1===due&&dep.a1Ready)||(dueLMX===due&&dep.lmxReady);return {type,due,dueA1,dueLMX,days,status,...dep,ready,activity:qcActivity(type,dep.phase)}}
...
function qcSelectedMedia(f){const out=[];if(f?.elements?.q3MediumA1?.checked)out.push('A1');if(f?.elements?.q3MediumLMX?.checked)out.push('LMX');return out}
function qcApplyMediumSelection({autoFromAvailability=false}={}){
  const f=$('#coliformQCForm');if(!f||!f.dataset.type)return;
  const type=f.dataset.type,box=$('#qcQ3MediumSelector');if(box)box.hidden=false;
  const title=$('#qcMediumSelectorTitle');if(title)title.textContent=`Medio(s) a controlar en ${type}`;
  const dep=qcDependency(type,f.elements.actualDate?.value||today());
  if(autoFromAvailability){
    const operative=f.dataset.mode!=='HISTORICAL',ref=f.elements.actualDate?.value||today();
    if(f.elements.q3MediumA1)f.elements.q3MediumA1.checked=!!dep.a1Ready&&(!operative||qcNextDueFor(type,'A1')<=ref);
    if(f.elements.q3MediumLMX)f.elements.q3MediumLMX.checked=!!dep.lmxReady&&(!operative||qcNextDueFor(type,'LMX')<=ref);
  }
  const selected=qcSelectedMedia(f),a1On=selected.includes('A1'),lmxOn=selected.includes('LMX');
...
$('#strainCatalogForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,o=Object.fromEntries(new FormData(f));const old=state.catalogStrains.find(x=>x.id===o.id);o.id=o.id||crypto.randomUUID();o.name=o.name.trim();o.referenceCode=o.referenceCode.trim();o.supplierLot=o.supplierLot.trim();o.shortCode=o.shortCode.trim().toUpperCase();o.incubationTemp=Number(o.incubationTemp);o.incubationHours=Number(o.incubationHours);o.cryovialLifeMonths=Number(o.cryovialLifeMonths);if(state.catalogStrains.some(x=>x.id!==o.id&&String(x.referenceCode||'').trim().toUpperCase()===String(o.referenceCode||'').trim().toUpperCase())){toast('Ya existe una cepa maestra con esa referencia ATCC. Registre el nuevo lote en Trazabilidad de productos → Recepción de lote y vincúlelo con esta cepa.');return}await saveLocal('catalogStrains',o);await audit('catalogStrain',o.id,old?'CEPA MAESTRA ACTUALIZADA':'CEPA MAESTRA REGISTRADA',{summary:`${o.name} · ${o.referenceCode} · ${o.supplierLot}`});f.reset();toast('Catálogo de cepas actualizado')});
...
$('#closureForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,p=state.mediaPrep.find(x=>x.id===f.elements.prepId.value),type=f.elements.closureType.value,reason=f.elements.closureReason.value.trim();if(!p){toast('Seleccione un lote');return}const releaseDecision=latestRelease(p.id)?.decision;if(!['LIBERADO','BLOQUEADO'].includes(releaseDecision)||isClosed(p)){toast(`El lote debe tener una liberación LIBERADO o BLOQUEADO antes de cerrarse. Liberación actual: ${releaseDecision||'—'}`);return}if(!type){toast('Seleccione el motivo real de cierre');return}if(type==='OTRO'&&!reason){toast('Describa el otro motivo');return}const closedAt=nowISO();await saveLocal('mediaPrep',{...p,status:'CERRADO',closureType:type,closureReason:reason,closureDate:closedAt.slice(0,10),closureAt:closedAt,closureResponsible:activeUser()},{render:false});await audit('mediaPrep',p.id,'LOTE CERRADO',{summary:`${closureLabel(type)}${reason?' · '+reason:''}`});await loadLocal();f.reset();toast('Cierre registrado con fecha, hora y responsable automáticos')});
